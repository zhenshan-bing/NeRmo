...
if __name__ == '__main__':
	...
	for n_fre in range(4):
		...
		def black_box(spine_angle, left_stride_length_scale_factor, right_stride_length_scale_factor):
			...
			INFO.append(info)
			return travelled_rad / 20

		optimizer = BayesianOptimization(f=black_box, pbounds=pbounds, random_state=1,allow_duplicate_points=True)
		optimizer.maximize(init_points=10, n_iter=100)

		opt_max.append({'max_angular_velocity': optimizer.max['target'],
						'fre': fre,
						'spine_angle': optimizer.max['params']['spine_angle'],
						'left_stride_length_scale_factor': optimizer.max['params'][
							'left_stride_length_scale_factor'],
						'right_stride_length_scale_factor': optimizer.max['params'][
							'right_stride_length_scale_factor'],
						'duty_factor': duty_factor})
	
	with open('bayes_opt_max_angular_velocity_df_'+str(duty_factor)+ '.csv', mode='w') as csv_file:
		fieldnames = ['max_angular_velocity', 'fre', 'spine_angle',
