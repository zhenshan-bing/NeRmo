...
class MouseController(object):
    ...
    def __init__(self, fre, time_step, spine_angle, front_left_scale_factor, front_right_scale_factor,
                 hind_right_scale_factor, hind_left_scale_factor, duty_factor):
        super(MouseController, self).__init__()
        PI = np.pi
        self.curStep = 0  # Spine

        self.front_left_scale_factor = front_left_scale_factor
        self.front_right_scale_factor = front_right_scale_factor
        self.hind_right_scale_factor = hind_right_scale_factor
    ...
    def getTurnSpineVal(self, spineStep):
        return self.spine_A

    def runStep(self):
        # FL
        foreLeg_left_q, swing_stance_state_fl = self.getLegCtrl(self.fl_left,
                                         self.curStep + self.stepDiff[0], 0, self.front_left_scale_factor, self.duty_factor)
        # FR
        foreLeg_right_q, swing_stance_state_fr = self.getLegCtrl(self.fl_right,
                                          self.curStep + self.stepDiff[1], 1, self.front_right_scale_factor, self.duty_factor)
        # HL
        hindLeg_left_q, swing_stance_state_hl = self.getLegCtrl(self.hl_left,
                                         self.curStep + self.stepDiff[2], 2, self.hind_left_scale_factor, self.duty_factor)
        # HR
        hindLeg_right_q, swing_stance_state_hr = self.getLegCtrl(self.hl_right,
                                          self.curStep + self.stepDiff[3], 3, self.hind_right_scale_factor, self.duty_factor)

        spineStep = self.curStep
        spine = self.getTurnSpineVal(spineStep)
        # spine = 0
    ...
    def runStepInit(self, timestep):
        foreLeg_left_q_first_timestep,_ = self.getLegCtrl(self.fl_left,
                                         self.stepDiff[0], 0, self.front_left_scale_factor, self.duty_factor)
        # FR
        foreLeg_right_q_first_timestep,_ = self.getLegCtrl(self.fl_right,
                                          self.stepDiff[1], 1, self.front_right_scale_factor, self.duty_factor)
        # HL
        hindLeg_left_q_first_timestep,_ = self.getLegCtrl(self.hl_left,
                                         self.stepDiff[2], 2, self.hind_left_scale_factor, self.duty_factor)
        # HR
        hindLeg_right_q_first_timestep,_ = self.getLegCtrl(self.hl_right,
                                          self.stepDiff[3], 3, self.hind_right_scale_factor, self.duty_factor)
        spine_q_first_timestep = self.getTurnSpineVal(0)
